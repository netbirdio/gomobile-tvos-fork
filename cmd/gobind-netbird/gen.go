...
func genPkg(lang string, p *types.Package, astFiles []*ast.File, allPkg []*types.Package, classes []*java.Class, otypes []*objc.Named) {
	...
	case "java":
		...
		if p == nil {
			dir, err := packageDir("github.com/netbirdio/gomobile-tvos-fork/bind")
			if err != nil {
				errorf(`"github.com/netbirdio/gomobile-tvos-fork/bind" is not found; run go get github.com/netbirdio/gomobile-tvos-fork/bind: %v`, err)
				return
			}
			repo := filepath.Clean(filepath.Join(dir, "..")) // github.com/netbirdio/gomobile-tvos-fork directory.
			...
			javaDir, err := packageDir("github.com/netbirdio/gomobile-tvos-fork/bind/java")
			if err != nil {
				errorf("unable to import bind/java: %v", err)
				return
			}
	...
	case "objc":
		...
		if p == nil {
			// Copy support files
			dir, err := packageDir("github.com/netbirdio/gomobile-tvos-fork/bind/objc")
			if err != nil {
