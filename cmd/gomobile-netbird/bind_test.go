...
func TestBindAndroid(t *testing.T) {
	...
	for _, tc := range tests {
		...
		cmdBind.flag.Parse([]string{"github.com/netbirdio/gomobile-tvos-fork/asset"})
		err := runBind(cmdBind)
		if err != nil {
			t.Log(buf.String())
			t.Fatal(err)
...
require github.com/netbirdio/gomobile-tvos-fork v0.0.0-20230905140555-fbe1c053b6a9
...
github.com/netbirdio/gomobile-tvos-fork v0.0.0-20230905140555-fbe1c053b6a9 h1:LaLfQUz4L1tfuOlrtEouZLZ0qHDwKn87E1NKoiudP/o=
github.com/netbirdio/gomobile-tvos-fork v0.0.0-20230905140555-fbe1c053b6a9/go.mod h1:2jxcxt/JNJik+N+QcB8q308+SyrE3bu43+sGZDmJ02M=
...
import (
	_ "github.com/netbirdio/gomobile-tvos-fork/app"
)
...
func TestBindWithGoModules(t *testing.T) {
	...
	if out, err := exec.Command("go", "build", "-o="+dir, "github.com/netbirdio/gomobile-tvos-fork/cmd/gobind-netbird").CombinedOutput(); err != nil {
		t.Fatalf("%v: %s", err, string(out))
	}
	if out, err := exec.Command("go", "build", "-o="+dir, "github.com/netbirdio/gomobile-tvos-fork/cmd/gomobile").CombinedOutput(); err != nil {
		t.Fatalf("%v: %s", err, string(out))
	}
	path := dir
	...
	for _, target := range []string{"android", "ios"} {
		...
		t.Run(target, func(t *testing.T) {
			...
			tests := []struct {
				...
				{
					Name: "Absolute Path",
					Path: "github.com/netbirdio/gomobile-tvos-fork/bind/testdata/cgopkg",
				},
				{
					Name: "Relative Path",
