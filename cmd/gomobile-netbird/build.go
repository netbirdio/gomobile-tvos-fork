...
func runBuildImpl(cmd *command) (*packages.Package, error) {
	...
	if !nmpkgs["github.com/netbirdio/gomobile-tvos-fork/app"] {
		return nil, fmt.Errorf(`%s does not import "github.com/netbirdio/gomobile-tvos-fork/app"`, pkg.PkgPath)
	}
...
func extractPkgs(nm string, path string) (map[string]bool, error) {
	if buildN {
		return map[string]bool{"github.com/netbirdio/gomobile-tvos-fork/app": true}, nil
	}
	r, w := io.Pipe()
