...
func runVersion(cmd *command) (err error) {
	// Check this binary matches the version in github.com/netbirdio/gomobile-tvos-fork/cmd/gomobile-netbird
	...
	version, err := func() (string, error) {
		...
		cmd := exec.Command("go", "list", "-f", "{{.Stale}}", "github.com/netbirdio/gomobile-tvos-fork/cmd/gomobile-netbird")
...
func mobileRepoRevision() (rev string, err error) {
	b, err := exec.Command("go", "list", "-f", "{{.Dir}}", "github.com/netbirdio/gomobile-tvos-fork/app").CombinedOutput()
	if err != nil {
